...
// Helper om de hoogste positie ooit te berekenen
function calculatePeakStat(targetName, type, targetArtist = null) {
    ...
    // Bepaal de klasse (goud voor nr 1)
    const rankClass = bestRank === 1 ? 'rank-1' : '';
    const rankDisplay = `#${bestRank}`;
    
    // Maak de weergave tekst: "3x #1" of "Mei '25 #4"
    const subText = occurrences > 1 ? `${occurrences}x behaald` : `in ${bestMonthLabel}`;

    return `
        <div class="peak-value-container">
            <span class="peak-rank ${rankClass}">${rankDisplay}</span>
...
function showArtistDetails(artist, overrideCount = null, monthKey = null, isBack = false) {
    ...
    if (peakStat) {
        html += `<p style="text-align:center; font-size:0.8rem; color:#ffcc00; margin-bottom:10px; font-weight:bold;">🏆 Hoogste Maandpositie: ${peakStat}</p>`;
    }
    ...
    if (topStreaks.length > 0) {
        ...
        topStreaks.forEach(st => {
            ...
            const periodStr = d1Obj.toLocaleDateString('nl-NL', {day:'numeric', month:'short'}) + " - " + d2Obj.toLocaleDateString('nl-NL', {day:'numeric', month:'short'});
            html += `<div class="streak-row" onclick="applyCalendarFilter('${escapeStr(cleanArtist)}', '${st.start}', '${st.end}', null)" style="cursor:pointer; transition:0.2s;"><span class="streak-days">${st.count} dagen</span><span class="streak-dates">${periodStr}</span></div>`;
        });
        html += `</div>`;
    }

    if (sortedAlbums.length > 0) {
        html += `<h3 style="font-size:0.9rem; color:#aaa; margin-bottom:10px; text-transform:uppercase;">Albums</h3><div style="display:flex; gap:12px; overflow-x:auto; padding-bottom:15px; margin-bottom:10px;">`;
        sortedAlbums.forEach((alb, idx) => {
            const albEscaped = escapeStr(cleanArtist);
            html += `<div onclick="handleAlbumClick('${alb.poster}', '${albEscaped}', '')" style="flex-shrink:0; cursor:pointer; width:80px; text-align:center;"><img src="${alb.poster}" style="width:80px; height:80px; border-radius:10px; object-fit:cover; margin-bottom:5px; box-shadow:0 4px 10px rgba(0,0,0,0.3);"></div>`;
        });
        html += `</div>`;
    }
...
function showSongSpotlight(songFull, overrideCount = null, isBack = false) {
    ...
    let html = `
        ...
        <div class="spotlight-stats-grid">
            ...
            <div class="spotlight-stat-box"><span class="spotlight-stat-val">${peakStat ? peakStat : '-'}</span><span class="spotlight-stat-label">Beste Positie</span></div>            <div class="spotlight-stat-box"><span class="spotlight-stat-val">${rank}</span><span class="spotlight-stat-label">Populair van ${artiest}</span></div>
            <div class="spotlight-stat-box"><span class="spotlight-stat-val">${percentage}</span><span class="spotlight-stat-label">van alle ${artiest} plays</span></div>
        </div>
    `;

    if (topStreaks.length > 0) {
        html += `<h3 style="font-size:0.9rem; color:#aaa; margin-bottom:10px; text-transform:uppercase; border-bottom:1px solid #333; padding-bottom:5px;">🔥 Beste Streaks</h3><div style="background:rgba(255,255,255,0.03); border-radius:10px; padding:5px;">`;
        topStreaks.forEach(st => {
            const d1Obj = new Date(st.start); const d2Obj = new Date(st.end);
            const d1 = d1Obj.toLocaleDateString('nl-NL', {day:'numeric', month:'short'});
            const d2 = d2Obj.toLocaleDateString('nl-NL', {day:'numeric', month:'short'});
            html += `<div class="streak-row" onclick="applyCalendarFilter('${escapeStr(artiest)}', '${st.start}', '${st.end}', '${escapeStr(titel)}')" style="cursor:pointer;"><span class="streak-days">${st.count} dagen</span><span class="streak-dates">${d1} ➔ ${d2}</span></div>`;
        });
        html += `</div>`;
    }
